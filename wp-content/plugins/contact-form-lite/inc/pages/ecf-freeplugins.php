...
<div class="wrap">
	<style>
	.easycform-plugin-action-buttons {
		text-align:right;
		}
	.easycform-plugin-action-buttons li {
		display: inline-block;
		margin-left: 1em;
		}
		
	.easycform-button-con {
		padding-right: 15px;
		}
		
.easycform-button-install {
				...
				switch ( $status['status'] ) {
					case 'install':
						if ( $status['url'] ) {
							...
							$action_links[] = '<a class="install-now button-secondary easycform-button-install" href="' . $status['url'] . '" aria-label="' . esc_attr( sprintf( __( 'Install %s now' ), $name ) ) . '">' . __( 'Install Now' ) . '</a>';
						}
					...
					case 'update_available':
						if ( $status['url'] ) {
							/* translators: 1: Plugin name and version */
							$action_links[] = '<a class="button easycform-button-update" href="' . $status['url'] . '" aria-label="' . esc_attr( sprintf( __( 'Update %s now' ), $name ) ) . '">' . __( 'Update Now' ) . '</a>';
						}
		...
		<div class="plugin-card">
			<div class="plugin-card-top" style="min-height: 160px !important;">
				...
				<div class="desc column-description" style="margin-right: 20px !important;">
					...
					<div class="easycform-button-con">
					<?php
						if ( $action_links ) {
							echo '<ul class="easycform-plugin-action-buttons">';
